...
def run_spider():
    while True:
        ...
        crawl, created = Crawl.get_or_create(Crawl.finished==False)
        if created:
            app = App.create(name='Start', start_url=str(crawl.get_id()))
        else:
            app = App.get(App.start_url==str(crawl.get_id()))

        while True:
            try:
                complated = parser.start(app, crawl)
                if complated:
                    crawl.finished = True
                    crawl.save()
                break
